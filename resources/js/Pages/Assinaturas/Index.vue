...
const props = defineProps({
    ...
    platformPlans: { type: Array, default: () => [] },
});
...
<template>
    <div class="space-y-6">
        ...
        <div v-if="platformPlans.length" class="rounded-xl border border-orange-500/30 bg-orange-500/5 p-4 dark:bg-orange-500/10">
            <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">Planos da plataforma</h2>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                Escolha uma assinatura para manter seu acesso ao painel e ao HiperCaixa.
            </p>
            <div class="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                <div v-for="plan in platformPlans" :key="plan.id" class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-800">
                    <p class="font-semibold text-zinc-900 dark:text-white">{{ plan.name }}</p>
                    <p class="mt-1 min-h-10 text-sm text-zinc-500 dark:text-zinc-400">{{ plan.description }}</p>
                    <p class="mt-3 text-2xl font-semibold text-zinc-900 dark:text-white">{{ formatBRL(plan.price) }}</p>
                    <a
                        v-if="plan.checkout_url"
                        :href="plan.checkout_url"
                        class="mt-4 inline-flex rounded-lg bg-[var(--color-primary)] px-4 py-2 text-sm font-semibold text-white hover:opacity-90"
                    >
                        Pagar assinatura
                    </a>
                </div>
